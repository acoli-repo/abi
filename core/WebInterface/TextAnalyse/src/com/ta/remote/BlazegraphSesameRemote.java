 ...
 *  --------------------------------------------------------------
 *   modifiziert durch : Kathrin Donandt, Zhanhong Huang
	...
	public static String queryExecutionGraph(String query) {
		...
		try {
			...
			outputfile = rwpp.getPropValues(Globals._PATH_TO_CONVERTER)
					+ "rdftriples.ttl";
			System.err.println("Triples: "+number_of_triples.toString());
			rwpp.writeProp("number_of_triples_rows", number_of_triples.toString(), Globals._MESSAGE_PATH);
			...
			String errorFile = rwpp.getPropValues("locationTA")+File.separator+"sparQLerrorFile.txt";
			List<String> lines = new ArrayList<String>();
			lines.add(e.toString());
			rwf.write(errorFile, lines);
...
//					Globals._MESSAGE_PATH);
	...
	public static String queryExecutionTuple(String query) {
		...
		try {
			...
			String path_to_visualize = rwpp.getPropValues(Globals._PATH_TO_VISUALIZE);
			outputfile = path_to_visualize + "triples.csv";
			System.err.println("Row: "+number_of_rows.toString());
			rwpp.writeProp("number_of_triples_rows", number_of_rows.toString(), Globals._MESSAGE_PATH);
...
//		decideQueryType(p);
//		String r = "SELECT ?t ?r {?t ?q ?r} LIMIT 100";
//		decideQueryType(r);
		
		String serviceURL = "http://localhost:9999/bigdata";
		RemoteRepositoryManager repo = new RemoteRepositoryManager(serviceURL,
				false /* useLBS */);
		loadDataFromResource(repo,"kb","/home/kathrin/Dokumente/TextAnalyse/Nanolib/data.n3");
